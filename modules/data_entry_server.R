...
data_entry_server <- function(id, reactive_data, pool, assembly_start_timestamp, is_assembly_running) {
  moduleServer(id, function(input, output, session) {
    ...
    # Observer to update the Tester's selectInput
    observe({
      updateSelectInput(session, "test_no", choices = available_for_testing())
    })
    
    # Observer to update the Packager's pickerInput
    observe({
      updatePickerInput(session, "pack_nos", choices = available_for_packaging())
    })
    ...
    output$packager_reprint_ui <- renderUI({
      if (length(input$packages_created_table_rows_selected) == 1) {
        actionButton(ns("reprint_packager_label"), "Reprint/Download Label", class = "btn-info")
      }
    })
    ...
    observeEvent(input$submit_package, {
      ...
      # (The rest of your packaging logic remains here...)
      
      # At the end of the logic, after creating packages:
      ...
      toastr_success("Packages created successfully!")
    })
    ...
    }, options = list(pageLength = 5, scrollX = TRUE), selection = 'single')
    ...
    output$packager_available_table <- DT::renderDataTable({
      # Wrap in a data.frame to ensure it's always a valid table format
